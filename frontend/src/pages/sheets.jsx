...
function Sheets() {
  ...
  const getColumns = () => {
    ...
    responses.forEach(r => r.answers?.forEach(a => { if (a.question?.text) allKeys.add(a.question.text); }));
    ...
    <div>
      ...
      {!loading && forms.length > 0 && (
        <div style={{ display: "flex", gap: "8px", marginBottom: "20px", flexWrap: "wrap" }}>
          {forms.map(form => (
            <button key={form.id} onClick={() => setSelectedForm(form)} style={{
      ...
      <div className="table-box">
        {loading ? (
          ...
          <p style={{ padding: "20px", color: "#6b7280" }}>Nuk ka përgjigje akoma për <strong>{selectedForm?.title}</strong>.</p>
        ) : (
          <table>
            <thead>
              <tr>
            ...
            <tbody>
              {responses.map((response, i) => (
                <tr key={response.id}>
                  ...
                  {columns.map((col, j) => <td key={j}>{getAnswer(response, col)}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
