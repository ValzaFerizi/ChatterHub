...
function Login() {
  ...
  const [welcomeMsg, setWelcomeMsg] = useState('Login to your account')
  const { login } = useAuth()
  ...
  useEffect(() => {
    api.get('/settings').then(res => {
      ...
      if (res.data.welcome_message) setWelcomeMsg(res.data.welcome_message)
    }).catch(() => {})
  }, [])
    ...
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', background: '#f8fafc' }}>
      <div className="panel" style={{ width: '100%', maxWidth: '420px' }}>
        <div className="page-header" style={{ textAlign: 'center' }}>
          <h1 style={{ color: '#6d28d9' }}>{appName}</h1>
          <p>{welcomeMsg}</p>
        </div>
