...
function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Login deshtoi')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="panel" style={{width:'100%',maxWidth:'420px'}}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="primary-btn" style={{width:'100%',padding:'12px',fontSize:'16px'}} disabled={loading}>
            {loading ? 'Duke u kyçur...' : 'Kycu'}
          </button>
        </form>
        <p style={{textAlign:'center',marginTop:'20px',fontSize:'14px',color:'#6b7280'}}>
          Nuk ke llogari? <a href="/register" style={{color:'#6d28d9',fontWeight:'600'}}>Regjistrohu</a>
        </p>
      </div>
    </div>
  )
